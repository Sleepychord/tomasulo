...
public class Logic implements LogicInterface {
	...
	public Logic(int nf, int nadd, int nmul, int nload, int nstore, int nru, int nmem){
		...
		for(int i = 0;i < nadd;i++)
			radd[i] = new CalcReservationStation("ADD" + Integer.toString(i+1));
		rmul = new CalcReservationStation[nmul];
		for(int i = 0;i < nmul;i++)
			rmul[i] = new CalcReservationStation("MULT" + Integer.toString(i+1));
		rload = new LoadReservationStation[nload];
		for(int i = 0;i < nload;i++)
			rload[i] = new LoadReservationStation("LOAD" + Integer.toString(i+1));
		rstore = new StoreReservationStation[nstore];
		for(int i = 0;i < nstore;i++)
			rstore[i] = new StoreReservationStation("STORE" + Integer.toString(i+1));
		ru = new int[nru];
		mem = new double[nmem];
		pc = 0;
		clk = 0;
